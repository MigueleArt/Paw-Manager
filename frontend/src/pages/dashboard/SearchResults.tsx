...
export default function SearchResults() {
  ...
  const counts = useMemo(() => {
    ...
    clients.forEach((c: any) => {
      const matchedCliente = c.pets.some(
        (p: any) => p.matchedBy?.includes('cliente') || p.matchedBy?.includes('email')
      );
  ...
  const filteredClients = useMemo(() => {
    ...
    if (tab === 'clientes') {
      return clients.filter((c: any) =>
        c.pets.some((p: any) => p.matchedBy?.includes('cliente') || p.matchedBy?.includes('email'))
      );
    }
    if (tab === 'telefonos') {
